package dev.jay.ecoclassify.data
...
class TfLiteClassifier(
    ...
    private fun setupClassifier() {
        val baseOptions = BaseOptions.builder().setNumThreads(2).useGpu().useNnapi().build()
        ...
        try {
            val modelPath = when (classifierType) {
                ClassifierType.FOOD -> "food.tflite"
                ClassifierType.PLANT -> "plants.tflite"
                ClassifierType.INSECT -> "insects.tflite"
                ClassifierType.BIRD -> "birds.tflite"
            }
